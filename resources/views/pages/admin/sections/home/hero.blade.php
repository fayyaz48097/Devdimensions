{{--
    SAVE AS: resources/views/pages/admin/sections/home/hero.blade.php
--}}
@extends('admin.admin')
...
@section('content')
    <div class="sec-wrap">
        ...
        <div class="sec-header">
            <div>
                <h1>Hero Section</h1>
                <p>Manage the main hero banner on the Home page.</p>
            </div>
            @if ($hero && !$hero->trashed())
                <div style="display:flex; gap:10px; flex-wrap:wrap;">
                    {{-- Toggle Status --}}
                    <form action="{{ route('admin.sections.home.hero.toggleStatus', $hero) }}" method="POST">
                        @csrf @method('PATCH')
                        @if ($hero->isActive())
                    ...
                    {{-- Soft Delete --}}
                    <form action="{{ route('admin.sections.home.hero.destroy', $hero) }}" method="POST"
                        onsubmit="return confirm('Soft-delete this hero section? The section will be hidden from the site.')">
                        @csrf @method('DELETE')
            ...
            <div class="fc">
                <div class="fc-body">
                    <div class="empty-state">
                        ...
                        <h3>Hero Section Deleted</h3>
                        <p>This section has been soft-deleted and is hidden from the public site.</p>
                        <form action="{{ route('admin.sections.home.hero.restore', $hero->id) }}" method="POST"
            ...
            <div class="fc" style="margin-bottom:20px;">
                <div class="fc-body">
                    <div class="empty-state" style="padding-bottom:10px;">
                        ...
                        <h3>No Hero Content Yet</h3>
                        <p>Fill in the form below to create the Hero Section for the Home page.</p>
                    </div>
                </div>
            </div>

            @include('pages.admin.sections.home._hero-form', ['hero' => null, 'isNew' => true])
    ...
    ══════════════════════════════════════ --}}
        @else
            {{-- Status banner --}}
            <div class="status-banner {{ $hero->status }}">
                @if ($hero->isActive())
            ...
            @include('pages.admin.sections.home._hero-form', ['hero' => $hero, 'isNew' => false])
