{{--
    SAVE AS: resources/views/pages/admin/sections/home/hireus_admin.blade.php
--}}

@php
    // Guard against double-encoded JSON stored in DB
    $checklistItems = $setting->checklist_items;
...
@extends('admin.admin')
@section('title', 'Home › Hire Us Section')
@section('page-title', 'Hire Us Section')
...
@section('content')
    <div class="sec-wrap">
        ...
        <div class="breadcrumb">
            ...
            <span class="sep">›</span><span class="current">Hire Us Section</span>
        </div>
        ...
        <div class="sec-header">
            <div>
                ...
                <p>Manage the left-column content and right-column hire boxes on the home page.</p>
            </div>
            {{-- Quick-toggle: passes all current setting values, only flips status --}}
            <form method="POST" action="{{ route('admin.sections.home.hireus.settings.update') }}">
                @csrf
                <input type="hidden" name="heading" value="{{ $setting->heading }}">
                <input type="hidden" name="subheading" value="{{ $setting->subheading }}">
                <input type="hidden" name="checklist_items" value="{{ implode("\n", $checklistItems) }}">
                <input type="hidden" name="cta_label" value="{{ $setting->cta_label }}">
                <input type="hidden" name="cta_url" value="{{ $setting->cta_url }}">
                ...
                <div class="fc">
                    <div class="fc-head">
                        <div class="fc-head-left">
                            ...
                            <div class="fc-title">Left Column Content</div>
                        </div>
                    </div>
                        ...
                        <form method="POST" action="{{ route('admin.sections.home.hireus.settings.update') }}">
                            ...
                            <div class="field">
                                ...
                                <input type="text" name="heading" class="fi"
                                    value="{{ old('heading', $setting->heading) }}" required maxlength="200">
                            </div>

                            <div class="field">
                                <label>Subheading Paragraph <span class="req">*</span></label>
                                <textarea name="subheading" class="fi" required maxlength="2000" style="min-height:90px;">{{ old('subheading', $setting->subheading) }}</textarea>
                            </div>

                            <div class="field">
                                <label>Checklist Items <span class="req">*</span> <span class="hint">one per
                                        line</span></label>
                                <textarea name="checklist_items" class="fi" required maxlength="500" style="min-height:80px;"
                                    placeholder="Hire Individual Resource&#10;Hire Multiple Resources&#10;Hire Entire Team or Department">{{ old('checklist_items', implode("\n", $checklistItems)) }}</textarea>
                                <div class="field-hint">Each line becomes one checklist bullet on the page.</div>
                            </div>

                            <div style="display:grid; grid-template-columns:1fr 1fr; gap:14px;">
                                <div class="field" style="margin-bottom:0;">
                                    <label>Button Label <span class="req">*</span></label>
                                    <input type="text" name="cta_label" class="fi"
                                        value="{{ old('cta_label', $setting->cta_label) }}" required maxlength="100"
                                        placeholder="Get Free Consultation">
                                </div>
                                <div class="field" style="margin-bottom:0;">
                                    <label>Button URL <span class="req">*</span></label>
                                    <input type="text" name="cta_url" class="fi"
                                        value="{{ old('cta_url', $setting->cta_url) }}" required maxlength="255"
                ...
                <div class="fc">
                    <div class="fc-head">
                        <div class="fc-head-left">
                            ...
                            <div>
                                <div class="fc-title">Hire Boxes</div>
                                <div class="fc-subtitle">Right column cards — drag to reorder</div>
                            </div>
                        </div>
                        <span class="step-count">{{ $boxes->whereNull('deleted_at')->count() }} boxes</span>
                    </div>

                    @if ($boxes->isEmpty())
                        <div class="empty-items">No boxes yet — add one using the form →</div>
                    @else
                        <ul class="step-list" id="sortable-boxes">
                            @foreach ($boxes as $index => $box)
                                <li class="step-row {{ $box->trashed() ? 'is-deleted' : '' }}"
                                    data-id="{{ $box->id }}">
                                    ...
                                    <div class="step-info">
                                        <div class="step-title-text">{{ $box->title }}</div>
                                        <div class="step-desc-text">{{ Str::limit($box->description, 65) }}</div>
                                    </div>

                                    @if ($box->trashed())
                                        <span class="pill pill-deleted">Deleted</span>
                                    @else
                                        <span class="pill pill-{{ $box->status }}">{{ ucfirst($box->status) }}</span>
                                    @endif

                                    <div class="item-actions">
                                        @if ($box->trashed())
                                            <form
                                                action="{{ route('admin.sections.home.hireus.boxes.restore', $box->id) }}"
                                                method="POST">
                                                @csrf
                                        ...
                                        @else
                                            <form
                                                action="{{ route('admin.sections.home.hireus.boxes.toggleStatus', $box) }}"
                                                method="POST">
                                                @csrf @method('PATCH')
                                                @if ($box->isActive())
                                            ...
                                            <button type="button" class="icon-btn btn-edit" title="Edit"
                                                ...
                                                {{ $box->id }},
                                                '{{ addslashes($box->title) }}',
                                                '{{ addslashes($box->description) }}',
                                                '{{ addslashes($box->box_url ?? '') }}',
                                                '{{ $box->status }}',
                                                {{ $box->imageUrl() ? "'" . addslashes($box->imageUrl()) . "'" : 'null' }},
                                                '{{ addslashes($box->image_original_name ?? '') }}'
                                            )">
                                                <svg width="13" height="13" viewBox="0 0 24 24" fill="none"
                                                    stroke="currentColor" stroke-width="2" stroke-linecap="round">
                                            ...
                                            <form action="{{ route('admin.sections.home.hireus.boxes.destroy', $box) }}"
                                                method="POST"
                                                onsubmit="return confirm('Soft-delete \'{{ addslashes($box->title) }}\'?')">
                                                @csrf @method('DELETE')
                ...
                <div class="fc">
                    <div class="fc-head">
                        <div class="fc-head-left">
                            ...
                            <div class="fc-title">Add New Box</div>
                        </div>
                    </div>
                    <div style="padding:20px;">
                        <form action="{{ route('admin.sections.home.hireus.boxes.store') }}" method="POST"
                            enctype="multipart/form-data">
                            @csrf

                            <div class="field">
                                <label>Image <span class="req">*</span> <span class="hint">SVG
                            ...
                            <div class="field">
                                ...
                                <input type="text" name="title"
                                    ...
                                    placeholder="e.g. Hire Team Member" required maxlength="150">
                                @error('title')
                                    <div class="field-error">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="field">
                                ...
                                <textarea name="description" class="fi {{ $errors->has('description') ? 'is-error' : '' }}" required
                                    maxlength="1000" placeholder="Short description shown on the card...">{{ old('description') }}</textarea>
                                @error('description')
                                    <div class="field-error">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="field">
                                <label>Box URL <span class="hint">(optional click destination)</span></label>
                                <input type="text" name="box_url" class="fi" value="{{ old('box_url') }}"
                                    placeholder="/contact-us" maxlength="255">
                            </div>
    ...
    <div class="modal-backdrop" id="edit-modal-backdrop">
        <div class="modal">
            <div class="modal-head">
                <span class="modal-title">Edit Hire Box</span>
            ...
            <div class="modal-body">
                ...
                <form id="edit-modal-form" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="field">
                        <label>Replace Image <span class="hint">(leave blank to keep current)</span></label>
                        <label class="file-upload-label">
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                stroke-width="2" stroke-linecap="round">
                                <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" />
                    ...
                    <div class="field">
                        ...
                        <input type="text" name="title" id="modal-title" class="fi" required maxlength="150"
                            placeholder="e.g. Hire Team Member">
                    </div>
                    ...
                    <div class="field">
                        <label>Box URL <span class="hint">(optional)</span></label>
                        <input type="text" name="box_url" id="modal-box-url" class="fi"
                            placeholder="/contact-us" maxlength="255">
                    </div>
...
@push('scripts')
    ...
    <script>
        (function() {
            ...
            // ── Drag-and-drop sort ──────────────────────────────────────────
            var list = document.getElementById('sortable-boxes');
            if (list) {
                Sortable.create(list, {
                    ...
                    onEnd: function() {
                        ...
                        fetch('{{ route('admin.sections.home.hireus.boxes.sort') }}', {
                            method: 'POST',
                            headers: {
            ...
            // ── Edit modal ──────────────────────────────────────────────────
            window.openEditModal = function(id, title, description, boxUrl, status, imgUrl, imgName) {
                var form = document.getElementById('edit-modal-form');
                var base =
                    '{{ rtrim(route('admin.sections.home.hireus.boxes.update', ['hireBox' => '__ID__']), '/') }}';
                form.action = base.replace('__ID__', id);

                document.getElementById('modal-title').value = title;
                document.getElementById('modal-description').value = description;
                document.getElementById('modal-box-url').value = boxUrl;
