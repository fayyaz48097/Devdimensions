{{--
    SAVE AS: resources/views/pages/admin/sections/about/joinnow.blade.php
--}}

@extends('admin.admin')
@section('title', 'About › Join Now Section')
@section('page-title', 'Join Now Section')
...
@section('content')
    <div class="sec-wrap">
        ...
        <div class="breadcrumb">
            ...
            <span class="sep">›</span><span class="current">Join Now Section</span>
        </div>

        {{-- Page header + quick-toggle --}}
        <div class="sec-header">
            <div>
                <h1>Join Now Section</h1>
                <p>Manage the logo, heading, description, and CTA button shown at the bottom of the About Us page.</p>
            </div>
            @if ($section && !$section->trashed())
                <form method="POST" action="{{ route('admin.sections.about.joinnow.toggleStatus', $section) }}">
                    @csrf @method('PATCH')
                ...
                <div class="fc">
                    <div class="fc-head">
                        <div class="fc-head-left">
                            ...
                            <div>
                                ...
                                <div class="fc-subtitle">Logo, heading, description and CTA button</div>
                            </div>
                        </div>
                        @if ($section)
                    ...
                    @if ($section && $section->trashed())
                        <div class="deleted-banner">
                            ...
                            <form action="{{ route('admin.sections.about.joinnow.restore', $section->id) }}" method="POST">
                                @csrf
                    ...
                    <div class="fc-body">
                        ...
                        <form id="save-form"
                            action="{{ $section && !$section->trashed()
                                ? route('admin.sections.about.joinnow.update', $section)
                                : route('admin.sections.about.joinnow.store') }}"
                            ...
                            <div class="field">
                                <label>
                                    Logo Image
                                    <span class="hint">SVG, PNG, WebP — max 2 MB — optional</span>
                                </label>

                                <div class="logo-preview-wrap" id="logo-preview-wrap">
                                    @if ($section?->logoUrl())
                                        <img id="logo-preview" src="{{ $section->logoUrl() }}"
                                            alt="{{ $section->logo_alt ?? '' }}">
                                    @else
                                        <div class="logo-preview-empty" id="logo-preview-empty">
                                ...
                                <label class="file-upload-label">
                                    ...
                                    <span id="logo-label-text">
                                        {{ $section?->logoUrl() ? 'Replace current logo' : 'Choose logo file' }}
                                    </span>
                                ...
                                @if ($section?->logo_original_name)
                                    <div style="font-size:11px; color:#333; margin-top:5px;">
                                        Current file: {{ $section->logo_original_name }}
                                    </div>
                                @endif
                                @error('logo')
                            ...
                            {{-- Logo alt text --}}
                            <div class="field">
                                <label>Logo Alt Text <span class="hint">for accessibility</span></label>
                                <input type="text" name="logo_alt" class="fi"
                                    placeholder="e.g. DevDimensions logo"
                                    value="{{ old('logo_alt', $section?->logo_alt) }}">
                            </div>
                            ...
                            <div class="field">
                                ...
                                <input type="text" name="heading"
                                    ...
                                    placeholder="e.g. Your turn to step up to the plate!"
                            ...
                            <div class="field">
                                ...
                                <textarea name="description" rows="4" class="fi {{ $errors->has('description') ? 'is-error' : '' }}"
                                    placeholder="Paragraph shown below the heading…">{{ old('description', $section?->description) }}</textarea>
                                @error('description')
                                    <div class="field-error">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div style="margin-top:22px; display:flex; gap:10px; align-items:center;">
                                ...
                                @if ($section && !$section->trashed())
                                    {{-- Trash button targets the separate delete form below via form= attribute.
                                     Nested <form> tags are invalid HTML and cause browsers to fire the wrong
                                     submission — this pattern is the correct fix. --}}
                        ...
                        {{-- ══ DELETE FORM — lives OUTSIDE the save form ══
                         The trash button above links here via form="delete-section-form".
                         Nested forms are invalid HTML; this keeps them completely separate. --}}
                        @if ($section && !$section->trashed())
                            <form id="delete-section-form"
                                action="{{ route('admin.sections.about.joinnow.destroy', $section) }}" method="POST"
                                style="display:none;">
                                @csrf @method('DELETE')
                            </form>
                        @endif
            ...
            <div>
                ...
                <div class="fc">
                    ...
                    <div class="fc-body"
                        ...
                        <p style="margin:0">• The <strong style="color:#555;">logo</strong> sits centered above the
                            heading — SVG recommended for crispness.</p>
                        <p style="margin:0">• <strong style="color:#555;">Heading</strong> is the large H2 text. Keep it
                            punchy — one sentence.</p>
                        <p style="margin:0">• <strong style="color:#555;">Description</strong> is the paragraph below the
                            heading.</p>
                        <p style="margin:0">• <strong style="color:#555;">Button URL</strong> can be a relative path like
                            <code style="color:#888;">/contact-us</code> or a full URL.</p>
                        ...
                        <p style="margin:0">• Max logo file size: <strong style="color:#555;">2 MB</strong>.</p>
                    </div>
                </div>
    ...
    <script>
        (function() {
            ...
            window.handleLogoChange = function(input) {
                ...
                var labelEl = document.getElementById('logo-label-text');
